...
這篇就是記錄這個過程。不是一次規劃好的，每一步都是獨立的 — 哪裡煩就先解決哪裡。

---

## 最初的麻煩點：要自己手動貼筆記到 Notion

跟 AI 討論的時候常常會聊出值得記錄的東西 — 一個觀念被釐清、一個之前沒想到的做法。當下的處理方式就是複製 AI 的回覆，貼到 Notion。

先不說**貼過去格式會跑掉，還全部都是沒整理過的原始對話，就都堆在那邊。** 日子一久根本不會回去看，記了也等於沒記。

有天跟人交流知道 Claude Code 可以寫指令（command），用 `/` 觸發讓 AI 做事。我就想 — 那我叫 AI 整理好再輸出不就好了？
...
跟 AI 討論後，決定用 GitHub Issue 來管。一開始也是手動 — 自己到網頁上開 issue、到要處理時再從 GitHub 貼回對話。這樣貼來貼去真的很麻煩，後來發現原來 GitHub 有出 CLI（命令列工具，可以在終端機直接操作 GitHub），於是我就寫了 `/record-issue`，讓 AI 根據對話直接幫我到 GitHub 開 Issue。
...
所以我寫了 `/note-review`，讓 AI 掃描整個筆記庫，分析哪些筆記是重複的、哪些是有關聯的，再對這些筆記做處理。另外我也把 `/note` 加了參數功能 — 帶上檔案路徑就能讀取我自己手寫的雜亂筆記，整理成同樣的結構化格式。

---

## 全貌

下面是目前整個工作流串起來的樣子，不用全部記住，看個大概就好。
...
flowchart TD
    ...
    B -- "自己寫的雜亂開發日誌<br/>(資料夾批次處理)" --> E["/note 檔案路徑/*.md<br/>整理為結構化 Obsidian 筆記"]
...
| Command             | 用途                                                      |
| ------------------- | --------------------------------------------------------- |
| `/commit`           | 分析變更，產生符合規範格式的 commit message |
...
| Command        | 用途                                                                           |
| -------------- | ------------------------------------------------------------------------------ |
| `/note`        | 從對話或指定檔案中提取洞察，產出結構化 Obsidian 筆記（支援帶路徑參數批次處理） |
...
這套工作流還有很多不完整的地方 — 沒有測試、文件寫法還在摸索、所有 command 都是手動觸發。但目前夠用，而且比以前好很多。
